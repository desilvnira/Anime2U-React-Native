...
class YetToWatch extends Component {
    state = {
        yetToWatchAnime: [],
        visible: false,
        dialogAnime: JSON
     ...
     */
    componentDidMount() {
        firebase.database().ref("/users/" + this.getUser() + "/yetToWatch").on('value', (snap) => {
            count = 0
            temp = []
            ...
            this.setState({ yetToWatchAnime: temp });
            count = 0
        })
    }
    ...
    moveAnime(option, anime) {
        if (option == 'remove') {
            firebase.database().ref("/users/" + this.getUser() + "/yetToWatch/" + anime.title).remove()
        }
        if (option == 'watching') {
            firebase.database().ref("/users/" + this.getUser() + "/yetToWatch/" + anime.title).remove()
        ...
        return (
            ...
            <View>
                <ScrollView>
                    ...
                    {
                        this.state.yetToWatchAnime.map((value => {
                            return (
                                <ListItem
                                    title={value.title}
                                    onPress={() => this.yetToWatchItemPressed(value)}
